...
As many of the different notions of relatedness
are connected to each other via some ``expectation''
(e.g., pedigree relatedness is the expected genetic identity within a pedigree),
we use the terms ``branch relatedness'' and ``branch GRM'' 
rather than ``eGRM'' or ``ARG-GRM'' here to avoid confusion
(see Methods for more discussion).
