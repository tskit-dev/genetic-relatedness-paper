...
Algorithm V follows a similar structure to previous incremental 
...
Fig.~\ref{fig:matvec-benchmark} where we plot the time taken to compute the 
branch GRM vector product against subsets of a large simulated 
ARG~\citep{andersontrocme2023genes}. Here, it takes only
17.8 seconds to run the \texttt{genetic\_relatedness\_vector} method 
for 1 million diploid samples. In contrast, computing the full branch
GRM using the \texttt{genetic\_relatedness\_matrix} method 
for \emph{twenty samples} required 28 seconds.
